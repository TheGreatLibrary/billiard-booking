...
{
    // Шаги: 1=место, 2=время, 3=столы(мульти), 4=эквип, 5=данные, 6=оплата, 7=успех
    public $step = 1;
    ...
    public function updateEquipmentQty($index, $qty)
    {
        ...
        if ($qty > $max) { $qty = $max; session()->flash('warning', "Доступно только {$max}"); }
    ...
    public function createPendingBooking(BookingService $service)
    {
        ...
        try {
            ...
            \Log::error('Guest booking error', ['message' => $e->getMessage()]);
