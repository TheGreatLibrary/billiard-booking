...
class BookingCreate extends Component
{
        ...
        foreach ($allResources as $resource) {
            foreach ($service->getAvailableSlots($resource, $this->date) as $time => $slot) {
                if (!isset($merged[$time])) {
                    $merged[$time] = ['available' => false, 'price' => PHP_INT_MAX, 'datetime' => $slot['datetime']];
                }
                if ($slot['available']) {
                    $merged[$time]['available'] = true;
                    $merged[$time]['price'] = min($merged[$time]['price'], $slot['price']);
                }
            }
        }
        foreach ($merged as &$s) { if (!$s['available']) $s['price'] = 0; }
        $this->availableSlots = $merged;
    }
    ...
    public function toggleSlot($time)
    {
        ...
        } else if (isset($this->availableSlots[$time]) && $this->availableSlots[$time]['available']) {
    ...
    {
        $this->selectedSlots = array_slice(
            array_keys(array_filter($this->availableSlots, fn($s) => $s['available'])), 0, $hours
        );
        $this->refreshAvailableResources();
        $this->calculateTotal();
    }
    ...
    private function recalculateResourcePrices()
    {
        ...
        $service = app(BookingService::class);
        foreach (Resource::with(['productModel', 'zone', 'place'])->whereIn('id', $this->availableResourceIds)->get() as $r) {
            $this->resourcePrices[$r->id] = $service->calculateResourcePrice($r, $this->date, $this->selectedSlots);
        }
    }
    ...
    public function toggleResource($resourceId)
    {
        ...
        if (!in_array($resourceId, $this->availableResourceIds)) {
            session()->flash('error', 'Стол недоступен');
            return;
        }
    ...
    {
        if (empty($this->selectedResources)) { session()->flash('error', 'Выберите минимум 1 стол'); return; }
    ...
    private function loadAvailableEquipment()
    {
        ...
        $this->availableEquipment = Resource::where('place_id', $this->place_id)
            ...
            ->map(fn($r) => [
                ...
                'available_qty' => $this->getAvailableEquipmentQty($r), 'total_qty' => $r->quantity,
            ])->filter(fn($eq) => $eq['available_qty'] > 0);
    }

    private function getAvailableEquipmentQty(Resource $r)
    {
        if (empty($this->selectedSlots) || !$this->date) return $r->quantity;
        $min = $r->quantity;
        foreach ($this->selectedSlots as $time) { $min = min($min, $r->getAvailableQuantity($this->date, $time)); }
        return $min;
    }
    ...
    public function addEquipment($resourceId)
    {
        foreach ($this->equipment as $item) {
            if ($item['resource_id'] == $resourceId) { session()->flash('warning', 'Уже добавлен'); return; }
        }
        $eq = collect($this->availableEquipment)->firstWhere('resource_id', $resourceId);
        if (!$eq || $eq['available_qty'] < 1) { session()->flash('error', 'Недоступен'); return; }
        $this->equipment[] = [
            'resource_id' => $eq['resource_id'], 'model_id' => $eq['model_id'],
            'name' => $eq['name'], 'price' => $eq['price'], 'qty' => 1, 'max_qty' => $eq['available_qty'],
        ];
        $this->calculateTotal();
    }
    ...
    public function createPendingBooking(BookingService $service)
    {
        ...
        try {
            $this->booking = $service->createPendingBooking([
                'user_id' => $userId, 'resource_ids' => $this->selectedResources,
                'date' => $this->date, 'slots' => $this->selectedSlots,
                'equipment' => $this->equipment,
                'guest_name' => $this->guest_name, 'guest_email' => $this->guest_email,
                'guest_phone' => $this->guest_phone, 'comment' => $this->comment,
            ]);
            $this->step = 6;
        } catch (\Exception $e) {
            \Log::error('Admin booking error', ['message' => $e->getMessage()]);
    ...
    {
        $total = 0;
        foreach ($this->selectedResources as $rid) { $total += $this->resourcePrices[$rid] ?? 0; }
        foreach ($this->equipment as $item) { $total += $item['price'] * $item['qty']; }
    ...
    public function getSelectedResourcesData(): array
    {
        ...
        foreach ($this->selectedResources as $id) {
            ...
            if ($r) { $r['price'] = $this->resourcePrices[$id] ?? 0; $result[] = $r; }
        }
        return $result;
    }
