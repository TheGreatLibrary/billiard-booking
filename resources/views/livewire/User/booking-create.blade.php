                        ...
                        @foreach($resources as $resource)
                            @php
                                ...
                                if (isset($resource['state'])) {
                                    $isAvailable = in_array(strtolower($resource['state']), ['available', 'доступен',  'active']);
                                }
                            @endphp
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            @foreach($availableEquipment as $eq)
                <div class="border-2 border-gray-200 dark:border-gray-700 rounded-xl p-6 bg-white dark:bg-gray-800 hover:border-blue-500 dark:hover:border-blue-400 transition-all duration-300 group hover:shadow-lg">
                    ...
                    <p class="text-gray-600 dark:text-gray-300 text-sm mb-4">Код: {{ $eq['code'] ?? '—' }}</p>

                    <div class="mb-3 p-2 bg-gray-50 dark:bg-gray-700 rounded text-center">
                        <span class="text-sm text-gray-700 dark:text-gray-200">
                            Доступно: <strong class="text-blue-600 dark:text-blue-400">{{ $eq['available_qty'] }}</strong> из {{ $eq['total_qty'] }}
                        </span>
                    </div>

                    <div class="flex items-center justify-between">
                        <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ number_format($eq['price'], 0) }} ₽</p>

                        <button wire:click="addEquipment({{ $eq['resource_id'] }})"
                                @disabled($eq['available_qty'] < 1)
                                class="px-4 py-2 rounded-lg font-medium transition-all duration-300 hover:scale-105
                                       {{ $eq['available_qty'] > 0
                    ...
                    <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-800 p-6 rounded-xl border-2 border-green-200 dark:border-green-800">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-green-600 dark:text-green-400 font-semibold">{{ number_format($item['price'], 0) }} ₽/шт</p>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Максимум доступно: {{ $item['max_qty'] }}</p>
                            </div>
                        </div>
